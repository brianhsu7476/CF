...
signed main(){
	...
	while(T--){
		...
		cin>>n>>m;
		rep(i, n)cin>>a[i];
		rep(i, n)rep(j, m)if(a[i][j]=='U')b.pb(P(i, j));
		rep(i, n)rep(j, m)if(a[i][j]=='L')c.pb(P(i, j));
		sort(all(b));
		sort(all(c), [](P a, P b){return a.y<b.y||a.y==b.y&&a.x<b.x;});
		bool ok=1;
		if(b.size()&1)ok=0;
		else rep(i, b.size()>>1)if(b[i*2].x!=b[i*2+1].x)ok=0;
		if(c.size()&1)ok=0;
		else rep(i, c.size()>>1)if(c[i*2].y!=c[i*2+1].y)ok=0;
		debug(b.size(), c.size(), ok);
		if(!ok){cout<<"-1\n"; continue;}
		rep(i, b.size()){
			P p=b[i];
			ans[p.x][p.y]=i&1, ans[p.x+1][p.y]=!(i&1);
		}
		rep(i, c.size()){
			P p=c[i];
			ans[p.x][p.y]=i&1, ans[p.x][p.y+1]=!(i&1);
		}
		rep(i, n){
			rep(j, m){
				if(a[i][j]=='.')cout<<'.';
				else cout<<(ans[i][j]?'B':'W');
			}
			cout<<endl;
		}
	}
