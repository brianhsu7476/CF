...
signed main(){
	...
	while(T--){
		...
		mx[n-1]=a[n-1];
		fdn(i, n-2, -1)mx[i]=max(mx[i+1], a[i]);
		int j=0;
		rep(i, n){
			for(; j<m&&b[j]>mx[i]; ++j)cout<<b[j]<<' ';
			cout<<a[i]<<' ';
		}
		for(; j<m; ++j)cout<<b[j]<<' ';
		cout<<endl;
		/*vi c;
		rep(i, n){
			int id=lower_bound(all(c), a[i])-c.begin();
			if(id<c.size())c[id]=a[i];
			else c.pb(a[i]);
		}
		int mx=c[c.size()-1], i=0;
		for(; i<m&&b[i]>mx; ++i)cout<<b[i]<<' ';
		rep(j, n)cout<<a[j]<<' ';
		for(; i<m; ++i)cout<<b[i]<<' ';
		cout<<endl;*/
	}
