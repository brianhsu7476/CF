...
signed main(){
	...
	while(T--){
		...
		fup(i, 1, n){
			now=++now%(mx[i]=mx[i-1]);
			if(~a[i]&&a[i]!=now)mx[i]=max(++mx[i], a[i]+1), now=a[i];
		}
		int mxn=mx[n-1];
		fdn(i, n-1, 0)if(~a[i]){
			if(mx[i-1]<mxn&&a[i]==mxn-1){
				if(mx[i-1]<--mxn&&!~a[i-1])a[i-1]=mxn-1;
			}
			else if(!~a[i-1])a[i-1]=(a[i]+mxn-1)%mxn;
		}
		mxn=1;
		fup(i, 1, n){
			if(~a[i])mxn=max(mxn, a[i]+1);
			else a[i]=(a[i-1]+1)%mxn;
		}
		cout<<mxn<<endl;
		rep(i, n)cout<<a[i]+1<<" \n"[i==n-1];
	}
