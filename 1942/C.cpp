...
signed main(){
	...
	while(T--){
		...
		sort(a, a+x);
		a[x]=a[0]+n;
		b.clear(), c.clear(), bu.clear(), bp.clear(), cu.clear(), cp.clear();
		int ans=n-2;
		rep(i, x)if(a[i+1]-a[i]>2){
			ans-=a[i+1]-a[i]-1;
			if(a[i+1]-a[i]&1)c.pb(a[i+1]-a[i]);
			else b.pb(a[i+1]-a[i]);
		}
		sort(b.begin(), b.end()), sort(c.begin(), c.end());
		bu.pb(0), bp.pb(0);
		rep(i, b.size())bu.pb(bu[i]+(b[i]-1)/2), bp.pb(bp[i]+b[i]-1);
		cu.pb(0), cp.pb(0);
		rep(i, c.size())cu.pb(cu[i]+(c[i]-1)/2), cp.pb(cp[i]+c[i]-1);
		int mx=0;
		fup(i, 0, bu.size()){
			int j=upper_bound(cu.begin(), cu.end(), y-bu[i])-cu.begin()-1;
			if(j==-1)continue;
			mx=max(mx, bp[i]+cp[j]+(y-bu[i]-cu[j])*2);
		}
		cout<<min(ans+mx, n-2)<<endl;
	}
}
