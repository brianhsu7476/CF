...
signed main(){
	...
	while(T--){
		cin>>n;
		inarr(c, c+n);
		fdn(i, n-2, -1)c[i]=min(c[i], c[i+1]);
		b.clear(), id.clear(), ans.clear();
		rep(i, n)if(i==n-1||c[i]<c[i+1])b.pb(c[i]), id.pb(i);
		fdn(i, b.size()-1, 0)b[i]-=b[i-1];
		orange(all(b));
		orange(all(id));
		rep(i, n)a[i]=0;
		cin>>k;
		int i=0, mnl=oo;
		while(i<b.size()&&k>=b[i]){
			int l=min(k/b[i], mnl);
			mnl=l;
			k-=l*b[i];
			ans.pb(l);
		...
		rep(i, ans.size()){
			fup(j, i?id[i-1]+1:0, id[i]+1)a[j]=ans[i];
		}
		rep(i, n)cout<<a[i]<<" \n"[i==n-1];
	}
}
