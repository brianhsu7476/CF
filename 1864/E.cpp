	...
	while(T--){
		...
		for(int b=30; b>-1; --b){
			for(int i=0, j=1; i<n; ++j){
				if(j==n||a[j]>>b!=a[j-1]>>b){
					if(a[i]>>b&1)ans=(ans+(j-i)*(j-i))%mod;
					i=j;
				}
			}
		}
		int eq=0;
		for(int i=0, j=1; i<n; ++j){
			if(j==n||a[j]!=a[j-1])eq=(eq+(j-i)*(j-i))%mod, i=j;
		}
		int all=n*n%mod;
		ans=(ans+all)%mod;
		ans=(ans+(all-eq)*Pow(2, mod-2, mod))%mod;
		ans=ans*Pow(all, mod-2, mod)%mod;
		ans=(ans+mod)%mod;
		cout<<ans<<endl;
	}
}
